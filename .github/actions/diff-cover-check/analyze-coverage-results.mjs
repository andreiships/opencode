...
async function checkCodeownersApproval(github, context) {
  ...
  const isSoleDev = codeowners.size === 1 && (codeowners.has(prAuthor) || codeowners.has(prAuthorBase))
  if (isSoleDev) {
...
function getCoverageData(reportPath = "coverage/diff-cover.json") {
  try {
    ...
    if (typeof data.total_percent_covered !== "number" || typeof data.total_num_lines !== "number") {
...
export async function analyzeCoverageResults({ reportPath = "coverage/diff-cover.json", threshold, context, github }) {
